...
export const readUser = async (req, res) => {
  ...
  if (user.id !== req.decoded.id && req.decoded.role !== "admin") {
    return res
      .status(403)
      .json({ message: "you are not allowed to view this user" });
  }
...
export const updateUserProfileStatus = async (req, res) => {
  ...
  // Only admin can update profile status
  if (req.decoded.role !== "admin") {
    return res
      .status(403)
      .json({ message: "you are not allowed to update profile status" });
  }

  const user = await editUserProfileStatus(id, status);
  ...
  res
    .status(200)
    .json({ message: "profile status updated successfully", data: user });
};
